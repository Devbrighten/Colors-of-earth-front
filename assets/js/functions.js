(function ($) {
    "use strict";
    function colors_video() {
        $('.buttonvideo').simpleShoesboxVideo();
    }
    function colors_google_maps() {
        if ($('.colors-google-maps').length <= 0) {
            return;
        }
        $('.colors-google-maps').each(function () {
            var $this = $(this),
                $id = $this.attr('id'),
    ...
    function colors_header_sticky($elem) {
        var $this = $elem;
        $this.on('colors_header_sticky', function () {
            $this.each(function () {
        ...
        }).trigger('colors_header_sticky');
        $(window).on('resize', function () {
            $this.trigger('colors_header_sticky');
        });
    }

    function colors_vertical_menu($elem) {
        /* SHOW ALL ITEM */
        var _countLi = 0,
    ...
    function colors_auto_width_vertical_menu() {
    ...
    function colors_animation_tabs($elem, _tab_animated) {
    ...
    function colors_init_carousel($elem) {
        $elem.not('.slick-initialized').each(function () {
            ...
            _this.on('init', function (event, slick, direction) {
                colors_popover_button();
            });
            _this.slick(_config);
        });
    }

    function colors_countdown($elem) {
        $elem.on('colors_countdown', function () {
            $elem.each(function () {
                var _this = $(this),
                    _text_countdown = '';
        ...
        }).trigger('colors_countdown');
    }

    // category product
    function colors_category_product($elem) {
        $elem.each(function () {
            var _main = $(this);
    ...
    function colors_magnific_popup() {
    ...
    function colors_better_equal_elems($elem) {
        $elem.each(function () {
    ...
    function colors_product_gallery($elem) {
        $elem.each(function () {
    ...
    function colors_hover_product_item($elem) {
        $elem.each(function () {
    ...
    function colors_popover_button() {
    ...
    $(window).on("load", function () {
        colors_video();
        colors_google_maps();
        if ($('.colors-threed-view').length > 0) {
            colors_threesixty();
        }
        if ($('.vertical-menu').length > 0) {
            colors_auto_width_vertical_menu();
        }
        if ($('.owl-slick').length) {
            $('.owl-slick').each(function () {
                colors_init_carousel($(this));
            });
        }
        if ($('.colors-countdown').length) {
            colors_countdown($('.colors-countdown'));
        }
        if ($('.product-gallery').length) {
            colors_product_gallery($('.product-gallery'));
        }
        if ($('.owl-slick .product-item').length) {
            colors_hover_product_item($('.colors-products .owl-slick .row-item,' +
                '.owl-slick .product-item'));
        }
        if ($('.block-nav-category').length) {
            colors_vertical_menu($('.block-nav-category'));
        }
        if ($('.widget_colors_nav_menu').length) {
            colors_vertical_menu($('.widget_colors_nav_menu'));
        }
        ...
        if ($('.widget_product_categories .product-categories').length) {
            colors_category_product($('.widget_product_categories .product-categories'));
        }
        if ($('.header-sticky .header-wrap-stick').length) {
            colors_header_sticky($('.header-sticky .header-wrap-stick'));
        }
        if ($('.equal-container.better-height').length) {
            colors_better_equal_elems($('.equal-container.better-height'));
        }
        colors_popover_button();
        colors_magnific_popup();
